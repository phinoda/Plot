...
export default function StatusView(_: { onCreate: () => void }) {
  ...
  // Fallback when no current project is set: pick the first project that
  // existed by activeDay. On today this is just projects[0]; on a past day
  // this skips projects that didn't exist yet.
  const project = useStore((s) => {
    if (s.currentProjectId) {
      const found = s.projects.find((p) => p.id === s.currentProjectId)
      if (found) return found
    }
    const day = s.viewingDay ?? todayKey()
    return s.projects.find((p) => todayKey(p.createdAt) <= day) ?? null
  })
  const entries = useStore((s) => {
    let id = s.currentProjectId
    if (!id) {
      const day = s.viewingDay ?? todayKey()
      id = s.projects.find((p) => todayKey(p.createdAt) <= day)?.id ?? null
    }
    return id ? (s.entriesByProject[id] ?? []) : []
  })
    ...
    >
      {hasMany && (
        <HitArea
          ...
          className="font-mono uppercase tracking-[0.2em] text-[10px] text-plot-ink/75 hover:text-plot-ink mb-6 transition-colors self-start"
        >
          <span>← all plots</span>
        </HitArea>
      )}
      ...
      <header className="mb-6 flex items-start justify-between gap-8 shrink-0">
        ...
        <div className="shrink-0 flex flex-col items-end gap-3">
          <HitArea
            onClick={askDelete}
            className="font-mono uppercase tracking-[0.2em] text-[12px] text-plot-ink/75 hover:text-red-900 transition-colors mb-6"
          >
      ...
      <div className="grid grid-cols-5 gap-10 flex-1 min-h-0">
        <Section title="Backlog" kind="backlog" entries={backlog}>
          <AddEntry kind="backlog" />
        </Section>
        <Section title="To-do" kind="todo" entries={todo}>
          <AddEntry kind="todo" />
        </Section>
        <Section title="Delivered" kind="delivered" entries={delivered} />
        <Section title="Decision" kind="decision" entries={decision}>
          <AddEntry kind="decision" />
        </Section>
        <Section title="Learning" kind="learning" entries={learning}>
          <AddEntry kind="learning" />
        </Section>
      </div>
    </main>
  )
